...
However, client tracking in Phase 1 was a rudimentary prototype baseline:

| Component | Phase 1 Implementation | Limitation / Problem to Solve in Phase 2 |
|---|---|---|
| `world/instance.rs` | `client_map: HashMap<SocketAddr, u64>` with implicit registration | No session tracking, no connection metadata, no player identity |
...
| Handshake / Auth | Implicit | No structured join handshake or client session lifecycle management |
...
2. **Protocol Extensions**: Add `JoinIntent` and `DisconnectIntent` to `proto/game_packets.proto`.
3. **Explicit Handshake Requirement**: Require an explicit `JoinIntent` before processing any gameplay intents (`MoveIntent`, `ActionIntent`, `PingIntent`). Unjoined packets from unknown `SocketAddr`s are dropped.
4. **Inactivity & Timeout Detection**: Check client heartbeats (`last_seen`) during the tick loop against a configurable timeout threshold (`CLIENT_TIMEOUT_SECS`, default: 10s) and cleanly despawn inactive entities.
5. **Graceful Disconnect & Despawning**: Cleanly remove entities and sessions from the simulation upon receiving `DisconnectIntent`.
6. **Zero-Overhead Simplified Handshake**: Maintain minimal architecture with zero external token/auth servers, optimized for rapid prototyping and indie/educational workflows.
...
```
        ...
        [ Known SocketAddr ]               [ Unknown SocketAddr ]
                  ...
                  │ Process Intent                    └─ Other Intent ────────► Drop Packet (No Session)
                  ▼
        ┌───────────────────┐
        │  Session: ACTIVE  │
        └─────────┬─────────┘
...
### 3.2 `src/world/session.rs` — Dedicated Session Model
...
Update `Entity` to store player display names:
...
### 3.5 `src/world/instance.rs` — Session & Lifecycle Management
...
- [x] **`proto/game_packets.proto`** — Add `JoinIntent` and `DisconnectIntent` messages, and register them in `ClientIntent.oneof`
- [x] **`build.rs` / `Cargo.toml`** — Verify `cargo check` compiles updated protobuf definitions with `prost`
- [x] **`src/world/session.rs`** — Implement `ClientSession`, `SessionState`, and `is_timed_out`
- [x] **`src/world/mod.rs`** — Register and export `pub mod session;`
- [x] **`src/world/entity.rs`** — Add `name: String` field to `Entity` and update `Entity::new` constructor
- [x] **`src/config.rs`** — Add `client_timeout_secs` (default `10`)
- [x] **`.env` / `.env.example`** — Document `CLIENT_TIMEOUT_SECS`
- [x] **`src/world/instance.rs`** — Implement `handle_join`, `handle_disconnect`, `check_timeouts`, and update `apply_intent` & `tick`
- [x] **`src/main.rs`** — Initialize `Instance` with timeout setting from `ServerConfig`
- [x] **`src/bin/client.rs`** — Add CLI commands for `join`, `leave`, `ping`, and auto-disconnect on `quit`
- [x] **Unit & Integration Tests** — Verify:
  ...
  - Unjoined client gameplay intents are safely dropped
  - End-to-end socket session lifecycle
- [x] **Client Examples** — Update Python, Love2D, and Godot examples to demonstrate explicit join/leave
...
5. **Strict Session Enforcement**:
   - Unjoined packets from unknown `SocketAddr`s sending raw `move` or `action` intents are safely dropped without polluting instance state.
