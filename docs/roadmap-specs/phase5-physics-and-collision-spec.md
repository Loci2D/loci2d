...
- [ ] **Milestone 5.1**: `DeterministicAABB`, `DeterministicCircle`, restoring `fixed_sqrt`, `deterministic_distance` (overflow-safe for distances $> 181$), and all 2D intersection queries implemented with 100% passing tests.
- [ ] **Milestone 5.2**: `MapBounds` (with `clamp_point`, `clamp_circle`, `clamp_aabb`) and `StaticObstacle` definitions integrated into `Instance` in `BTreeMap` order.
